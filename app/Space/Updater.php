...
class Updater
{
    ...
    public static function download($new_version, $is_cmd = 0)
    {
        ...
        $response = static::getRemote($url, ['timeout' => 500, 'track_redirects' => true]);
        // Exception
    ...
    public static function copyFiles($temp_extract_dir)
    {
        ...
        if (! File::copyDirectory($temp_extract_dir.'/qbh-retails', base_path())) {
            return false;
        }
    ...
    {
        /** Update events for extra opreation commented for temp */
        //event(new UpdateFinished($installed, $version));

        /** directly added to update only version to settings  */
