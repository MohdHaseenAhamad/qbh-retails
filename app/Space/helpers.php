 ...
 */
function format_money_pdf($money, $currency = null, $currency_show = true)
{
    $money = $money / 100;

    if (! $currency) {
        $currency = Currency::findOrFail(CompanySetting::getSetting('currency', Company::orderBy('id')->first()->id));
    }
    ...
    if ($currency->swap_currency_symbol) {
        if($currency_show){
            ...
            $currency_with_symbol = $format_money;
        }
    } else {
        if($currency_show){
