    ...
    <form v-if="!isPreview" action="">
      <div class="px-8 py-8 sm:p-6">
        <BaseInputGrid layout="one-column" class="col-span-7">
          ...
          >
            <BaseInput
              v-model="proformaMailForm.subject"
              type="text"
...
const proformaMailForm = reactive({
  id: null,
  from: null,
  to: null,
  subject: 'New Proforma',
  body: null,
})

const modalActive = computed(() => {
  return modalStore.active && modalStore.componentName === 'SendProformaModal'
})
...
async function setInitialData() {
  ...
  proformaMailForm.id = modalStore.id

  if (admin.data) {
    proformaMailForm.from = admin.data.from_mail
  }

  if (modalData.value) {
    proformaMailForm.to = modalData.value.customer.email
  }

  proformaMailForm.body = companyStore.selectedCompanySettings.proforma_mail_body
}
...
async function submitForm() {
  ...
  try {
    ...
    if (!isPreview.value) {
      const previewResponse = await proformaStore.previewProforma(proformaMailForm)
    ...
    const response = await proformaStore.sendProforma(proformaMailForm)

    if (response.data.success) {
      closeSendProformaModal()
      return true
    }
  } catch (error) {
    console.error(error)
    ...
    notificationStore.showNotification({
      type: 'error',
      message: t('proformas.something_went_wrong'),
    })
  }
}

function closeSendProformaModal() {
