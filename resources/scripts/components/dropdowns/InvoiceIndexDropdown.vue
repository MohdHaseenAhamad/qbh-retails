<template>
  <BaseDropdown class="z-20">
    ...
    <BaseDropdownItem v-show="row.status == 'SENT' || row.status == 'COMPLETED'" @click="generateCreditNote(row)">
      <BaseIcon
        name="PlusIcon"
        class="w-5 h-5 mr-3 text-gray-400 group-hover:text-gray-500"
      />
...
function generateCreditNote(row) {
  router.push(`/admin/credit-note/${row.id}/create`)
}

function canSendInvoice(row) {
  return (
    (row.status == 'DRAFT' ||  row.status == 'OVERDUE') &&
    route.name !== 'invoices.view' &&
    userStore.hasAbilities(abilities.SEND_INVOICE)
  )
}
