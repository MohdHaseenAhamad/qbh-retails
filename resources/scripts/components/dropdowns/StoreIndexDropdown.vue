<template>
  <BaseDropdown :content-loading="supplierStore.isFetchingViewData">
    <template #activator>
      <BaseButton v-if="route.name === 'stores.view'" variant="primary">
    ...
    <router-link
      v-if="
        ...
        userStore.hasAbilities(abilities.VIEW_STORE)
      "
      :to="`stores/${row.id}/view`"
    >
      <BaseDropdownItem>
        <BaseIcon
          name="EyeIcon"
    ...
    <!-- Delete Customer  -->
    <BaseDropdownItem
      v-if="userStore.hasAbilities(abilities.DELETE_STORE)"
      @click="removeStore(row.id)"
    >
      <BaseIcon
        name="TrashIcon"
        class="w-5 h-5 mr-3 text-gray-400 group-hover:text-gray-500"
...
function removeStore(id) {
  ...
  dialogStore
    .openDialog({
      ...
      message: t('stores.confirm_delete', 1),
    ...
    .then((res) => {
      if (res) {
        supplierStore.deleteStore({ ids: [id] }).then((response) => {
