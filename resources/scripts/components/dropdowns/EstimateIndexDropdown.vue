<template>
  <BaseDropdown class="z-20">
    <template #activator>
      <BaseButton v-if="row.is_edit != '0' && route.name === 'estimates.view'" variant="primary">
    ...
    <!-- Copy PDF url  -->
    <BaseDropdownItem
      v-if="row.is_edit != '0' && route.name === 'estimates.view'"
      @click="copyPdfUrl"
    >
      <BaseIcon
        name="LinkIcon"
    ...
    <!-- Edit Estimate -->
    <router-link
      v-if="row.is_edit != '0' && userStore.hasAbilities(abilities.EDIT_ESTIMATE)"
    ...
    <BaseDropdownItem
      v-if="row.is_edit != '0' && userStore.hasAbilities(abilities.CREATE_INVOICE) && row.status == 'ACCEPTED'"
    ...
    <!-- Resend Estimate -->
    <BaseDropdownItem v-if="row.is_edit != '0' && canResendEstimate(row)" @click="sendEstimate(row)">
      <BaseIcon
        name="PaperAirplaneIcon"
        class="w-5 h-5 mr-3 text-gray-400 group-hover:text-gray-500"
      />
...
async function reviseQuotation(data) {
  dialogStore
    .openDialog({
      title: t('general.are_you_sure'),
      message: t('estimates.confirm_revise'),
      yesLabel: t('general.ok'),
      noLabel: t('general.cancel'),
      variant: 'primary',
      hideNoButton: false,
