    ...
    >
      <div class="flex relative justify-between">
        <BaseText
          ...
          :length="30"
          class="overflow-hidden flex-1 text-base font-medium text-left text-gray-900"
        />
        <div class="flex" v-if="isEditable">
          <a v-show="false"
            class="
              relative
              my-0
          ...
          >
            ...
            >
              ...
              <label
                ...
                "
              >
                {{ props.type === 'purchase' ? $t('suppliers.add_new_supplier') : $t('customers.add_new_customer') }}
              </label>
            </button>
          </PopoverPanel>
        </div>
...
const selectedCustomer = computed(() => {
  switch (props.type) {
    case 'estimate':
      isEditable.value = (estimateStore.newEstimate.is_edit ? (estimateStore.newEstimate.is_edit == '1' ? true : false) : true)
      return estimateStore.newEstimate.customer
    case 'invoice':
      isEditable.value = (invoiceStore.newInvoice.is_edit || invoiceStore.newInvoice.from_quotation == '0' ? (invoiceStore.newInvoice.is_edit == '1' && invoiceStore.newInvoice.from_quotation == '0' ? true : false) : true)
      return invoiceStore.newInvoice.customer
    case 'credit':
      isEditable.value = false
      return creditStore.newInvoice.customer
    case 'proforma':
      isEditable.value = (proformaStore.newProforma.is_edit ? (proformaStore.newProforma.is_edit == '1' ? true : false) : true)
      return proformaStore.newProforma.customer
    case 'purchase':
      isEditable.value = true
      return purchaseStore.newPurchase.customer
    case 'debit':
      isEditable.value = false
      return debitStore.newPurchase.customer
...
function resetSelectedCustomer() {
  if (props.type === 'estimate') {
    ...
    creditStore.resetSelectedCustomer()
  } else if (props.type === 'proforma') {
    proformaStore.resetSelectedCustomer()
  } 
  else if (props.type === 'purchase') {
    purchaseStore.resetSelectedCustomer()
  } 
  else if (props.type === 'debit') {
    debitStore.resetSelectedCustomer()
  } 
  else {
...
else if (props.customerId && props.type === 'credit') {
  creditStore.selectCustomer(props.customerId)
} 
else if (props.customerId && props.type === 'proforma') {
  proformaStore.selectCustomer(props.customerId)
} 
else if (props.customerId && props.type === 'purchase') {
  purchaseStore.selectCustomer(props.customerId)
} 
else {
...
async function fetchInitialCustomers() {
  ...
  await customerStore.fetchCustomers(data, props.type)
}
...
function selectNewCustomer(id, close) {
  ...
  if (props.type === 'estimate') {
    ...
    proformaStore.selectCustomer(id)
  } 
  else if (props.type === 'purchase') {
    purchaseStore.selectCustomer(id)
  } 
  else{
    creditStore.selectCustomer(id, props.type)
  }
  close()
  search.value = null
}
