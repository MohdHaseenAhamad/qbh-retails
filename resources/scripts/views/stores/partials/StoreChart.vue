<template>
  <BaseCard class="flex flex-col mt-6">
    <!-- <StorePlaceholder v-if="supplierStore.isFetchingViewData" />
...
import StorePlaceholder from './StoreChartPlaceholder.vue'
...
async function loadSupplier() {
  ...
  let response = await supplierStore.fetchViewStore({
    id: route.params.id,
  })
    if (response.data) {
...
async function onChangeYear(data) {
  ...
  let response = await supplierStore.fetchViewStore(params)
