<template>
  ...
  >
    ...
    <div
      ...
      "
    >
      <div v-for="(supplier, index) in supplierStore.suppliers" :key="index">
        <router-link
          v-if="supplier && !isFetching"
          :id="'supplier-' + supplier.id"
          :to="`/admin/stores/${supplier.id}/view`"
          :class="[
        ...
        >
          <div>
            ...
            <BaseText
              v-if="supplier.address_street_1 || supplier.address_street_2 || supplier.city || supplier.state || supplier.zip"
              :text="
              (supplier.address_street_1?supplier.address_street_1:'') + ' ' + 
              (supplier.address_street_2?supplier.address_street_2:'') + ' ' + 
      ...
      <p
        v-if="!supplierStore.suppliers.length && !isFetching"
...
async function loadSuppliers() {
  ...
  await supplierStore.fetchStores({ limit: 'all' })
...
async function onSearch() {
  ...
  try {
    let response = await supplierStore.fetchStores(data)
    isSearching.value = false
    if (response.data) {
      supplierStore.suppliers = response.data.data
    }
  } catch (error) {
    isSearching.value = false
  }
