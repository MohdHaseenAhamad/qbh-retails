<template>
  <BasePage>
    <!-- Page Header Section -->
    <BasePageHeader :title="$t('stores.title')">
      <BaseBreadcrumb>
      ...
      <template #actions>
        <div class="flex items-center justify-end space-x-5">
          <BaseButton
            v-show="supplierStore.totalCustomers"
    ...
    <BaseFilterWrapper :show="showFilters" class="mt-5" @clear="clearFilter">
      ...
      <BaseInputGroup :label="$t('stores.code')" class="text-left">
        <BaseInput
    ...
    <BaseEmptyPlaceholder
      ...
      :title="$t('stores.no_stores')"
      :description="$t('stores.list_of_stores')"
    >
      ...
      <template #actions>
        <BaseButton
          v-if="userStore.hasAbilities(abilities.CREATE_CUSTOMER)"
          variant="primary-outline"
          @click="$router.push('/admin/stores/create')"
        >
          <template #left="slotProps">
            <BaseIcon name="PlusIcon" :class="slotProps.class" />
          </template>
...
function removeMultipleSuppliers() {
  dialogStore
    .openDialog({
      ...
      message: t('stores.confirm_delete', 2),
    ...
    .then((res) => {
      if (res) {
        supplierStore.deleteMultipleStores().then((response) => {
          if (response.data) {
            refreshTable()
          }
        })
