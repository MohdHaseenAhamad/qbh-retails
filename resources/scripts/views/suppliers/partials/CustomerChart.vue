<template>
  <BaseCard class="flex flex-col mt-6">
    <!-- <ChartPlaceholder v-if="supplierStore.isFetchingViewData" />
...
async function loadSupplier() {
  ...
  let response = await supplierStore.fetchViewSupplier({
    id: route.params.id,
  })
    if (response.data) {
...
async function onChangeYear(data) {
  ...
  let response = await supplierStore.fetchViewSupplier(params)
