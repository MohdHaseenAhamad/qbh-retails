<template>
  <BasePage>
    ...
    <BaseFilterWrapper :show="showFilters" class="mt-5" @clear="clearFilter">
      <BaseInputGroup :label="$tc('items.name')" class="text-left w-1/3">
        <BaseInput
      ...
      <div class="pt-6" v-if="false">
      <router-link
        v-if="filteredDataCount"
        :to="`/filtered-items-list?selectedCompany=${companyStore.selectedCompany.id}&name=${filters.name}`"
        target="_blank"
      >
...
const itemColumns = computed(() => {
  return [
    ...
    { key: 'category_name', label: t('categories.name') },
