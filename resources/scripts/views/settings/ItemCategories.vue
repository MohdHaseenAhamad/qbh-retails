<template>
  ...
  <BaseSettingCard
    :title="$t('settings.categories.items.heading')"
    :description="$t('settings.categories.items.description')"
  >
    <template #action>
      <BaseButton
    ...
    >
      <template #cell-actions="{ row }">
        <CategoryDropdown
          ...
          modal-title="settings.categories.items.edit"
        />
      </template>
    </BaseTable>
  </BaseSettingCard>
...
async function openNoteSelectModal() {
  await modalStore.openModal({
    title: t('settings.categories.items.add_category'),
