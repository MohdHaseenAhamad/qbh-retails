<template>
  <form @submit.prevent="submitForm">
    <h6 class="text-gray-900 text-lg font-medium">
      {{ $t('settings.customization.credits.default_formats') }}
    </h6>
    <p class="mt-1 text-sm text-gray-500 mb-2">
      {{ $t('settings.customization.credits.default_formats_description') }}
    </p>

    <BaseInputGroup
      :label="$t('settings.customization.credits.default_credit_email_body')"
      class="mt-6 mb-4"
    >
      <BaseCustomInput
...
async function submitForm() {
  ...
  await companyStore.updateCompanySettings({
    data,
    message: 'settings.customization.credits.credit_settings_updated',
  })
