<template>
  <form action="" class="relative" @submit.prevent="updatePreferencesData">
    ...
    >
      ...
      <ul class="divide-y divide-gray-200">
        <BaseSwitchSection
          v-model="deductionPerItemField"
          :title="$t('settings.preferences.deduction_per_item')"
          :description="$t('settings.preferences.deduction_setting_description')"
        />
      </ul>
    </BaseSettingCard>
  </form>
  ...
  set: async (newValue) => {
    ...
    let data = {
      settings: {
        ...
        deduction_per_item : (newValue ? 'NO' : 'YES')
      },
    }
        settingsForm.deduction_per_item= (newValue ? 'NO' : 'YES'),
...
const deductionPerItemField = computed({
  get: () => {
    return settingsForm.deduction_per_item === 'YES'
  },
  set: async (newValue) => {
    const value = newValue ? 'YES' : 'NO'
    ...
    let data = {
      settings: {
        deduction_per_item: value,
        discount_per_item : (newValue ? 'NO' : 'YES')
      },
    }
    settingsForm.discount_per_item= (newValue ? 'NO' : 'YES'),

    settingsForm.deduction_per_item = value
