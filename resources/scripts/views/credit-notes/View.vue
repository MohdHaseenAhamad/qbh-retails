...
import InvoiceDropdown from '@/scripts/components/dropdowns/CreditNoteIndexDropdown.vue'
...
const pageTitle = computed(() => invoiceData.value.credit_number)
...
const shareableLink = computed(() => {
  return `/credits/pdf/${invoiceData.value.unique_hash}`
})
...
async function loadInvoices() {
  ...
  await creditStore.fetchCreditNotes()
...
async function loadInvoice() {
  let response = await creditStore.fetchCreditNote(route.params.id)
  if (response.data) {
...
async function onSearched() {
  ...
  let response = await creditStore.searchInvoice(data)
  isSearching.value = false
  if (response.data) {
    creditStore.invoices = response.data.data
  }
}
...
<template>
  ...
  <BasePage v-if="invoiceData" class="xl:pl-96 xl:ml-8">
    <BasePageHeader :title="pageTitle">
      <template #actions>
        <div class="text-sm mr-3">
          <BaseButton
            v-if="invoiceData.is_edit_credit == '1' &&
    ...
    >
          ...
          <BaseDropdown class="ml-3" position="bottom-start">
            ...
            <BaseDropdownItem class="flex px-1 py-2 cursor-pointer">
              <BaseInputGroup class="-mt-3 font-normal">
                <BaseRadio
                  id="filter_credit_date"
                  v-model="searchData.orderByField"
                  :label="$t('reports.credits.date')"
                  size="sm"
                  name="filter"
                  value="credit_date"
            ...
            <BaseDropdownItem class="flex px-1 py-2 cursor-pointer">
              <BaseInputGroup class="-mt-3 font-normal">
                <BaseRadio
                  id="filter_credit_number"
                  v-model="searchData.orderByField"
                  :label="$t('credits.number')"
                  value="credit_number"
                  size="sm"
                  name="filter"
                  @update:modelValue="onSearched"
      ...
      <div
        v-if="creditStore && creditStore.invoices"
        class="
          h-full
          pb-32
          overflow-y-scroll
        ...
        "
      >
        <div v-for="(invoice, index) in creditStore.invoices" :key="index">
          <router-link
            v-if="invoice && !isLoading"
            :id="'invoice-' + invoice.id"
            :to="`/admin/credit-note/${invoice.id}/view`"
            :class="[
        ...
        <p
          v-if="!creditStore.invoices.length && !isLoading"
