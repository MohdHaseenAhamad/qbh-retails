  ...
  <BasePage class="relative payment-create">
    <form action="" @submit.prevent="submitPaymentData">
      <BasePageHeader :title="pageTitle" class="mb-5">
        ...
        <template #actions v-if="paymentStore.currentPayment.invoice ? paymentStore.currentPayment.invoice.due_amount != 0 : true">
          <BaseButton
            :loading="isSaving"
            :disabled="isSaving"
            variant="primary"
        ...
        <BaseInputGrid>
          ...
          <BaseInputGroup
            ...
            :help-text="
              selectedInvoice
                ? `Due Amount: ${
                    (paymentStore.currentPayment.invoice ? paymentStore.currentPayment.invoice.due_amount  : paymentStore.currentPayment.maxPayableAmount) / 100
                  }`
                : ''
            "
            required
            :error="
              v$.currentPayment.invoice_id.$error &&
              v$.currentPayment.invoice_id.$errors[0].$message
            "
          >
            <BaseMultiselect
          ...
          <BaseInputGroup
            :label="$t('payments.recieved_amount')"
            :content-loading="isLoadingContent"
            :error="
...
const discount = computed({
  get: () => paymentStore.currentPayment.discount / 100,
  set: (value) => {
    paymentStore.currentPayment.discount = Math.round(value * 100)
  },
})
...
const rules = computed(() => {
  console.log(paymentStore.currentPayment,'invoiceStore.totalinvoiceStore.total  ')
  return {
    currentPayment: {
      customer_id: {
      ...
      amount: {
        ...
        //   between(1, paymentStore.currentPayment.invoice ? (paymentStore.currentPayment.invoice.total - discount) : paymentStore.currentPayment.maxPayableAmount)
        // ),
      },
      invoice_id: {
        required: helpers.withMessage(t('validation.required'), required),
...
function onCustomerChange(customer_id) {
  if (customer_id) {
    let data = {
      ...
      doublestatus: 'SENTOVERDUE',
      limit: 'all'
    }
console.log(isEdit.value,'isEdit.valueisEdit.valueisEdit.value')
    if (isEdit.value) {
      data.ownid = paymentStore.currentPayment.invoice_id
    }
