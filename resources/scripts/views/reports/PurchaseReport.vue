  ...
  <div class="grid gap-8 md:grid-cols-12 pt-10">
    <div class="col-span-8 md:col-span-4">
      ...
      <div class="flex flex-col my-6 lg:space-x-3 lg:flex-row">
        ...
        <BaseInputGroup :label="$t('general.sort_by_order')">
            <BaseMultiselect
              v-model="formData.sort_by_order"
              :groups="true"
              :options="sort_by_order"
              :placeholder="$t('general.select_a_sort_by_order')"
              @update:modelValue="setSortBy"
              @remove="clearStatusSearch()"
            />
        </BaseInputGroup>
...
const sort_by = ref([
  {
    label: 'Status',
    options: ['DATE', 'PURCHASE_NUMBER'],
  }
])

const sort_by_order = ref([
  {
    label: 'Sort By Order',
    options: ['ASC', 'DESC'],
  }
...
const customerDateRangeUrl = computed(() => {
  return `${customerSiteURL.value}?from_date=${moment(
    ...
    'YYYY-MM-DD'
  )}&sortBy=${formData.sort_by}&sort_by_order=${formData.sort_by_order}`
})
...
watch(range, (newRange) => {
  ...
  formData.sort_by = 'DATE',
  formData.sort_by_order = 'ASC'
})

onMounted(() => {
  customerSiteURL.value = `/reports/purchase/suppliers/${getSelectedCompany.value.unique_hash}`
...
function setSortBy(val) {
  switch (val) {
    ...
    case 'PURCHASE_NUMBER':
      filters.sort_by = 'PURCHASE_NUMBER'
      break
    default:
      filters.sort_by = 'DATE'
      break
...
function setSortByOrder(val) {
  switch (val) {
    case 'ASC':
      filters.sort_by_order = 'ASC'
      break
    case 'DESC':
      filters.sort_by_order = 'DESC'
      break
    default:
      filters.sort_by_order = 'ASC'
      break
