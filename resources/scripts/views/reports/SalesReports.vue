       ...
       <div class="flex flex-col my-6 lg:space-x-3 lg:flex-row">
          <BaseInputGroup :label="$t('general.sort_by_order')">
            <BaseMultiselect
              ...
              :placeholder="$t('general.sort_by_order')"
              @update:modelValue="setSortByOrder"
              @remove="clearStatusSearch()"
            />
        </BaseInputGroup>
      </div>
...
function setSortBy(val) {
  switch (val) {
    case 'DATE':
      ...
      break
    case 'INVOICE_NUMBER':
      formData.sort_by = 'INVOICE_NUMBER'
      break
    default:
      formData.sort_by = 'DATE'
      break
...
function setSortByOrder(val) {
  console.log('setSortByOrder',val)
  switch (val) {
    case 'ASC':
      formData.sort_by_order = 'ASC'
      break
    case 'DESC':
      formData.sort_by_order = 'DESC'
      break
    default:
      formData.sort_by_order = 'ASC'
      break
