...
export const useEstimateStore = (useWindow = false) => {
  ...
  return defineStoreFunc({
    ...
    getters: {
      ...
      getSubtotalWithDiscount() {
        if(this.newEstimate.discount_type == 'percentage')
          return this.getSubTotal - Math.round(this.getSubTotal * this.newEstimate.discount / 100)

        return this.getSubTotal - (this.newEstimate.discount * 100)
      },
    ...
    actions: {
      ...
      deselectBankDetail() {
        this.newEstimate.bank_detail_id = ''
      },
      ...
      deselectPreparedBy() {
        this.newEstimate.prepared_by_id = ''
      },
        ...
        return new Promise((resolve, reject) => {
          axios
            ...
            .then((response) => {
              ...
              notificationStore.showNotification({
                type: 'success',
                message: global.t('estimates.revised_successfully'),
              })
              resolve(response)
            })
            .catch((err) => {
      ...
      fetchEstimateTemplates(params) {
        return new Promise((resolve, reject) => {
          axios
            ...
            .then((response) => {
              this.templates = globalStore.setAllowedTempletes(response.data.estimateTemplates)
              resolve(response)
            })
            .catch((err) => {
              handleError(err)
