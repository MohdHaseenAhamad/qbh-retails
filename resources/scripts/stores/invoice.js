...
export const useInvoiceStore = (useWindow = false) => {
  ...
  return defineStoreFunc({
    ...
    getters: {
      ...
      getSubtotalWithDiscount() {
        if(this.newInvoice.discount_type == 'percentage')
          return this.getSubTotal - Math.round(this.getSubTotal * this.newInvoice.discount / 100)

        return this.getSubTotal - (this.newInvoice.discount * 100)
      },
    ...
    actions: {
      ...
      fetchInvoices(params, forcreditnote = false) {
        return new Promise((resolve, reject) => {
          axios
            .get(`/api/v1/${forcreditnote ? 'invoices/sent_invoices_not_in_credit_note' : 'invoices'}`, { params })
            .then((response) => {
        ...
        return new Promise((resolve, reject) => {
          axios
            ...
            .then((response) => {
              ...
              this.newInvoice.prepared_by_id = response.data.data.prepared_by_id
              resolve(response)
            })
            .catch((err) => {
              handleError(err)
      ...
      deselectBankDetail() {
        this.newInvoice.bank_detail_id = ''
      },
      ...
      async fetchInvoiceInitialSettings(isEdit) {
        ...
        Promise.all([
          ...
          this.getNextNumber('withInvoiceDate'),
          ...
          .then(async ([res1, res2, res3, res4, res5, res6]) => {
            if (!isEdit) {
              if (res4.data) {
                ...
                this.newInvoice.invoice_date = moment(res4.data.invoiceDate).format('YYYY-MM-DD H:mm:ss')
              }
