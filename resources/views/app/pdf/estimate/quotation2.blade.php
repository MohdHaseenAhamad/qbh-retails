        ...
        <table width="100%">
            <tr>
                <td width="60%" class="header-section-left">
                    ...
                    @else
                        <h1 class="header-logo"> {{$estimate->comp_name}} </h1>
                    @endif
                    @if($estimate->comp_name)
                        <h2 class="header-company-name"> {{$estimate->comp_name}} </h2>
                    @endif
                </td>
    ...
    <div class="wrapper" style="padding-top: 130px;">
        <div class="company-details-container">
            ...
            <div class="company-address-container company-address">
                {{$estimate->comp_address_street_1.', '.$estimate->comp_city.', '.$estimate->comp_state.', '.$estimate->comp_zip}}
                <br>
                {{$estimate->comp_phone}}
            </div>
        ...
        <div class="customer-address-container">
            <h5 class="client-address-heading">Quotation for :</h5>
            <p style="font-size: 12px;width:50%;">{{$estimate->subject ?? 'NA'}}</p>
            <div class="billing-address-container billing-address">
                ...
                @endif
                <br>
                {{$estimate->cus_address_street_1.', '.$estimate->cus_city.', '.$estimate->cus_state.', '.$estimate->cus_zip}}
                <br>
                {{$estimate->cus_phone}}
            </div>
            <div class="estimate-details-container">
        ...
        <div class="customer-address-container">
            <h5 class="instruction-heading">Comments or Special Instructions</h5>
            <p class="instruction-paragraph">Dear Sir, With reference to your enquiry, please find our competitive Quotation as follow: </p>
            <div class="estimate-details-container">
            </div>
            <div style="clear: both;"></div>
            ...
            <div width="100%">
                <table class="bank_table" width="100%">
                    ...
                    <tr>
                        ...
                        <td class="border">
                            <span class="arabic" >{{$estimate->comp_account_name_ar != null ? $estimate->comp_account_name_ar.' / ' : ''}}</span>
                             {{$estimate->comp_account_name != null ? $estimate->comp_account_name : ''}}</td>
                    </tr>
                    <tr>
                        <th class="border th_left"><span class="arabic" >{{\Lang::get('BANK NAME',[],'ar')}}</span> / {{\Lang::get('Bank Name',[],'en')}}</th>
                        <td class="border">
                            <span class="arabic" >{{$estimate->comp_bank_name_ar != null ? $estimate->comp_bank_name_ar.' / ' : ''}}</span>{{$estimate->comp_bank_name != null ? $estimate->comp_bank_name :''}}</td>
                    </tr>
                    <tr>
                        <th class="border th_left"><span class="arabic" >{{\Lang::get('ACCOUNT No',[],'ar')}}</span> / {{\Lang::get('Account No',[],'en')}}</th>
                        <td class="border"><span class="arabic" >{{$estimate->comp_account_no_ar != null ? $estimate->comp_account_no_ar.' / ' : ''}}</span>{{$estimate->comp_account_no != null ? $estimate->comp_account_no : ''}}</td>
                    </tr>
                    <tr>
                        <th class="border th_left"><span class="arabic" >{{\Lang::get('IBAN',[],'ar')}}</span> / {{\Lang::get('IBAN',[],'en')}}</th>
                        <td class="border"><span class="arabic" >{{$estimate->comp_iban_ar != null ? $estimate->comp_iban_ar.' / ' : ''}}</span>{{$estimate->comp_iban != null ? $estimate->comp_iban : ''}}</td>
                    </tr>
                    <tr>
                        <th class="border th_left"><span class="arabic" >{{\Lang::get('SWIFT CODE',[],'ar')}}</span> / {{\Lang::get('Swift Code',[],'en')}}</th>
                        <td class="border"><span class="arabic" >{{$estimate->comp_swift_code_ar != null ? $estimate->comp_swift_code_ar.' / ' : ''}}</span>{{$estimate->comp_swift_code != null ? $estimate->comp_swift_code : ''}}</td>
                    </tr>
                </table>
